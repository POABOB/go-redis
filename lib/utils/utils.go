...
// ToCommandLine2 convert commandName and string-type argument to [][]byte
func ToCommandLine2(commandName string, args ...string) [][]byte {
...
// ToCommandLine3 convert commandName and []byte-type argument to CommandLine
func ToCommandLine3(commandName string, args ...[]byte) [][]byte {
	result := make([][]byte, len(args)+1)
	result[0] = []byte(commandName)
	for i, s := range args {
		result[i+1] = s
